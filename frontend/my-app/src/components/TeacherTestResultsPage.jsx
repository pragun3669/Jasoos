...
const TeacherTestResultsPage = () => {
  ...
  const normalizeDate = (value) => {
    ...
    try {
      ...
      // Trim nanoseconds → keep only 3 digits (milliseconds)
      if (str.includes(".")) {
        const [datePart, msPart] = str.split(".");
        const ms = msPart.substring(0, 3); // keep first 3 digits only
        str = `${datePart}.${ms}`;
      }
  ...
  const fetchTeacherTests = async () => {
    try {
      ...
      const testsWithCounts = await Promise.all(
        data.map(async (test) => {
          try {
            const resultsResponse = await fetch(`${API_BASE_URL}/tests/${test.id}/results`, {
              headers: { Authorization: `Bearer ${token}` }
            });
  ...
  const fetchTestResults = async (testId) => {
    try {
      ...
      const processedData = data.map(result => {
        ...
        // Calculate total score as AVERAGE of all question scores (each question is out of 100)
        let totalScore = 0;
        if (questionResults.length > 0) {
          const sumOfScores = questionResults.reduce((sum, q) => {
            // Each question score is already out of 100 (earnedPoints)
            return sum + (q.earnedPoints || 0);
          }, 0);
          totalScore = sumOfScores / questionResults.length; // Average
        }

        // Process each question to ensure correct/incorrect status matches score
        const processedQuestionResults = questionResults.map(q => {
          const earnedPoints = q.earnedPoints || 0;
          const questionMarks = q.questionMarks || 100;
          
          // A question is correct if student earned 100% of marks
          const isCorrect = earnedPoints >= questionMarks;
          ...
          return {
            ...
            percentage: questionMarks > 0 ? (earnedPoints / questionMarks) * 100 : 0
          };
        });
        ...
        return {
          ...
          totalMarks: 100,   // Total is always out of 100
                  ...
                  <div className="px-6 pb-6">
                    <div className="space-y-3 mb-6">
                      ...
                      <div className="flex items-center text-sm text-gray-600 dark:text-gray-400">
                        ...
                        <span>Created: {formatDate(test.createdDate)}</span>
                      </div>
                    </div>
                                            ...
                                            <div className="flex items-center justify-between">
                                              ...
                                              <div className="text-right">
                                                <div className={`text-2xl font-bold ${
                                                  qResult.correct ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'
                                                }`}>
                                          ...
                                          <div className="p-5 space-y-4">
                                            ...
                                            <div className="space-y-2">
                                              ...
                                              <div className="flex items-center justify-between text-xs">
                                                ...
                                                <span className={`font-semibold ${
                                                  qResult.correct ? 'text-green-600 dark:text-green-400' : 'text-orange-600 dark:text-orange-400'
                                                }`}>
                                            ...
                                            {qResult.submittedCode && (
                                              <button
                                                onClick={() => {
                                                  ...
                                                  setExpandedCode(prev => prev.includes(key) ? prev.filter(k => k !== key) : [...prev, key]);
                                            ...
                                            {expandedCode.includes(`${result.studentId}-q${idx}`) && (
                                              <div className="mt-4 space-y-4 border-t-2 border-gray-200 dark:border-gray-700 pt-4 animate-fadeIn">
                                                {qResult.submittedCode && (
                                                  <div className="space-y-2">
                                                    <div className="flex items-center justify-between">
                                                          ...
                                                          <div className="p-3 space-y-3">
                                                            {tc.input && (
                                                              <div>
                                                                ...
                                                                <pre className="bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 p-2 rounded text-xs overflow-x-auto font-mono">
                                                                  {tc.input}
                                                                </pre>
                                                              </div>
                                                            )}
                                                            ...
                                                            <div className="grid grid-cols-2 gap-3">
                                                              {tc.expectedOutput && (
                                                                <div>
                                                                  ...
                                                                  <pre className="bg-green-100 dark:bg-green-900/30 border border-green-300 dark:border-green-700 p-2 rounded text-xs overflow-x-auto font-mono text-green-900 dark:text-green-200">
                                                                    {tc.expectedOutput}
                                                                  </pre>
                                                                </div>
                                                              )}
                                                            ...
                                                            {tc.error && (
                                                              <div>
                                                                ...
                                                                <pre className="bg-red-100 dark:bg-red-900/30 border-2 border-red-400 dark:border-red-700 p-2 rounded text-xs overflow-x-auto text-red-900 dark:text-red-200 font-mono">
                                                                  {tc.error}
                                                                </pre>
                                                              </div>
                                                            )}
                                        ...
                                        <div className="flex items-center justify-between">
                                          <div>
                                            ...
                                            <p className="text-xs text-blue-600 dark:text-blue-400 mt-1">
                                              out of {result.questionResults?.length || 0}
                                            </p>
                                          </div>
